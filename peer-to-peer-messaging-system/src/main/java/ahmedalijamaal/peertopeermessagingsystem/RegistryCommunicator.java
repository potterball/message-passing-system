...
public class RegistryCommunicator {
    ...
    /**
     * Buffered reader that reads requests from the socket connected to the
     * registry.
     */
    ...
    /**
     * Constructor that sets up information needed to send the connection request to
     * the registry.
     * 
    ...
    private void sendCode() throws Exception {
        ...
        printFiles(".src/main.java/ahmedalijamaal/peertopeermessagingsystem");
    ...
    private void receivePeers() throws Exception {
        ...
        for (int i = 0; i < numOfPeers; i++) {
            // received a peer and checks if it's information is redundant(already sent
            // before and recorded)
            ...
            new_peer = new Peer(new ProcessAddress(peer_info[0], port),
                    new ProcessAddress(
                            ((InetSocketAddress) socket.getRemoteSocketAddress()).getAddress().toString().split("/")[1],
                            socket.getPort()),
                    timestamp);
            ...
            // checks if received peer is already a member of the set of active peer,
            // updates information
    ...
    /**
     * Sends a report of it's actions to the registry. This should include the peers
     * stored in
     * the set of peers, peers received from other peers and the messages received.
     * 
     * @throws Exception if there is any problems communicating with the peer for
     *                   this report request.
     */
    private void sendReport() throws Exception {
        // sending the number of active peers
        writer.write(source.current_peers.size() + "\n");
        // sending the ip and port number of the active peers. Format:
        // <ip><colon><port><newline>
        ...
        // sending the ip and port number of the peers received from the registry.
        // Format: <ip><colon><port><newline>
        ...
        // Format:
        // <sourceIP><colon><sourcePort><space><receivedIP><colon><receivedPort><space><time
        // received><newline>
        ...
        // Format:
        // <destinationIP><colon><destinationPort><space><sentIP><colon><sentPort><space><time
        // received><newline>
        ...
        // Format:
        // <lamportTimestamp><space><content><space><sourceIP><colon><sourcePort><newline>
     ...
     * send: <name of team><new line>
     * 
     * @throws Exception if there is any problems communicating with the registry or
     *                   retrieving local ip
     */
    private void sendLocation() throws Exception {
        writer.write(InetAddress.getLocalHost().getHostAddress() + ":" + source.getLocalPort() + "\n");
        writer.flush();
    }
}
